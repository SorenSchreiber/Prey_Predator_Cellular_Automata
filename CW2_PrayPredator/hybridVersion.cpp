...
#define threadNumber 4					//thread number

void runhybridVersion(int argc, char** argv, int xHeight, int yHeight)
{
	cout << "Running HYBRID version with X=" << xHeight << ",Y=" << yHeight << ", ThreadNumber=" << threadNumber << endl;

	bool arrayIsSquare = false;				//is grid square
	bool XBigSide = true;					//is x big side

	int xseg = xHeight;						//internal grid x lenth
	int yseg = yHeight;						//internal grid y length
	int messageSize = yHeight;				//communication message length
...
#pragma omp for
			...
			count++;			//increase generation counter
		}
		...
		diff = ((float)end - (float)start);		//calcualte runtime

		std::cout << endl << "Runtime: " << diff / 60000 << endl;	//display runtime
	}
	ReleaseDC(console, dc);				//release resources

	MPI_Barrier(MPI_COMM_WORLD);		//barrier
	std::cout << "FINILIZE" << endl;	//print finalize
